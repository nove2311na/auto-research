...
from research_pipeline.tools.artifact_io import build_meta, write_meta
from research_pipeline.tools.manifest import finalize, init_manifest, is_done, record_attempt
...
extract_winner = {
    ...
    "facts": [
        ...
        {"claim": "Real-time decoding latency remains the bottleneck for fault-tolerant gates", "evidence_quote": "real-time decoding latency", "confidence": "medium"},
    ],
    "quotes": [],
}
...
synth_artifact = {
    ...
    "theses": [
        {"statement": "Surface codes will dominate industrial QEC for the next 3-5 years",
         "evidence": ["Surface codes have ~1% threshold", "1000+ logical qubits demonstrated in 2026"],
        ...
        {"statement": "Decoder latency, not qubit count, is the real bottleneck to fault tolerance",
         "evidence": ["Real-time decoding latency remains the bottleneck for fault-tolerant gates"],
...
format_json = {
    "yaml_frontmatter": {
        "title": TOPIC,
        "tags": ["quantum", "error-correction", "research-report"],
        "aliases": ["QEC 2026"],
    },
...
def render_md(f: dict) -> str:
    md = [f"# Research Report: {TOPIC}", "", "## Summary", "", f["summary"], "",
          "## Key Insights", ""]
...
for stage, (artifact, ext, producer) in stages_artifacts.items():
    ...
    (sdir / f"v1.{ext}").write_text(json.dumps(artifact, indent=2), encoding="utf-8")
...
(out / "05_format" / "v1.md").write_text(md, encoding="utf-8")
...
md_text = (out / "05_format" / "v1.md").read_text(encoding="utf-8")
...
ingest_text_on_disk = json.loads((out / "01_ingest" / "v1.json").read_text(encoding="utf-8"))["text"]
...
ingest_meta_on_disk = json.loads((out / "01_ingest" / "v1.json").read_text(encoding="utf-8"))["metadata"]
...
fmt_on_disk = json.loads((out / "05_format" / "v1.json").read_text(encoding="utf-8"))
