...
from research_pipeline.tools.artifact_io import OUTPUTS, REPO, atomic_write, now_iso


def _load_pipeline() -> dict[str, Any]:
    return cast(dict[str, Any], json.loads((REPO / "pipeline.json").read_text(encoding="utf-8")))
...
def read_manifest(input_id: str) -> dict[str, Any] | None:
...
def write_manifest(input_id: str, manifest: dict[str, Any]) -> Path:
    p = manifest_path(input_id)
